...
public class TestXmlCharReader extends TestCase {
    ...
    protected void _testRawNoBomValid(String encoding) throws Exception {
        ...
        XmlCharReader XmlCharReader = new XmlCharReader(is, false);
        assertEquals(XmlCharReader.getEncoding(), "UTF-8");

        is = getXmlStream("no-bom", XML2, encoding, encoding);
        XmlCharReader = new XmlCharReader(is);
        assertEquals(XmlCharReader.getEncoding(), "UTF-8");

        is = getXmlStream("no-bom", XML3, encoding, encoding);
        XmlCharReader = new XmlCharReader(is);
        assertEquals(XmlCharReader.getEncoding(), encoding);
    ...
    protected void _testRawBomValid(String encoding) throws Exception {
        ...
        if (!encoding.equals("UTF-16")) {
            ...
            assertEquals(XmlCharReader.getEncoding().substring(0, encoding.length()), encoding);
        }
    }
    ...
    protected void _testRawBomInvalid(String bomEnc, String streamEnc, String prologEnc) throws Exception {
        ...
        try {
            XmlCharReader XmlCharReader = new XmlCharReader(is, false);
            fail("It should have failed for BOM " + bomEnc + ", streamEnc " + streamEnc + " and prologEnc " +
                 prologEnc);
        }
    ...
    public void _testAlternateDefaultEncoding(String cT, String bomEnc, String streamEnc, String prologEnc,
                                              String alternateEnc) throws Exception {
        try {
            InputStream is = getXmlStream(bomEnc, (prologEnc == null) ? XML1 : XML3, streamEnc, prologEnc);
            XmlCharReader.setDefaultEncoding(alternateEnc);
            XmlCharReader XmlCharReader = new XmlCharReader(is, cT, false);
            if (!streamEnc.equals("UTF-16")) {
                // we can not assert things here becuase UTF-8, US-ASCII and ISO-8859-1 look alike for the chars
                // used for detection
            }
            else {
                String enc = (alternateEnc != null) ? alternateEnc : streamEnc;
                assertEquals(XmlCharReader.getEncoding().substring(0, streamEnc.length()), streamEnc);
            }
        }
        finally {
            XmlCharReader.setDefaultEncoding(null);
        }
    }
    ...
    public void _testHttpValid(String cT, String bomEnc, String streamEnc, String prologEnc) throws Exception {
        ...
        XmlCharReader XmlCharReader = new XmlCharReader(is, cT, false);
        if (!streamEnc.equals("UTF-16")) {
            // we can not assert things here becuase UTF-8, US-ASCII and ISO-8859-1 look alike for the chars used for
            // detection
        }
        else {
            assertEquals(XmlCharReader.getEncoding().substring(0, streamEnc.length()), streamEnc);
    ...
    protected void _testHttpInvalid(String cT, String bomEnc, String streamEnc, String prologEnc) throws Exception {
        ...
        try {
            new XmlCharReader(is, cT, false);
            fail("It should have failed for HTTP Content-type " + cT + ", BOM " + bomEnc + ", streamEnc " + streamEnc +
                 " and prologEnc " + prologEnc);
        }
        catch (XmlCharReaderException ex) {
            assertTrue(ex.getMessage().indexOf("Invalid encoding,") > -1);
        }
    ...
    protected void _testHttpLenient(String cT, String bomEnc, String streamEnc, String prologEnc, String shouldbe)
            throws Exception {
        InputStream is = getXmlStream(bomEnc, (prologEnc == null) ? XML2 : XML3, streamEnc, prologEnc);
        XmlCharReader XmlCharReader = new XmlCharReader(is, cT, true);
        assertEquals(XmlCharReader.getEncoding(), shouldbe);
    }
    ...
    public void testEncodingAttributeXML() throws Exception {
        ...
        XmlCharReader XmlCharReader = new XmlCharReader(is, "", true);
        assertEquals(XmlCharReader.getEncoding(), "UTF-8");
    }
    ...
    private static final MessageFormat XML_WITH_PROLOG_AND_ENCODING_DOUBLE_QUOTES =
            new MessageFormat("<?xml version=\"1.0\" encoding=\"{1}\"?>\n<root>{2}</root>");
    ...
    private static final MessageFormat XML_WITH_PROLOG_AND_ENCODING_SPACED_SINGLE_QUOTES =
            new MessageFormat("<?xml version=\"1.0\" encoding =  \t \n \r''{1}''?>\n<root>{2}</root>");
